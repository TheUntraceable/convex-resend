...
const PERMANENT_ERROR_CODES = new Set([
    400, 401 /* 402 not included - unclear spec */, 403, 404, 405, 406, 407,
    408,
...
export const makeBatch = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (
            emails.length === 0 ||
            (args.reloop && emails.length < BATCH_SIZE)
        ) {
...
export const handleEmailEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        const email = await ctx.db
            .query("emails")
            .withIndex("by_resendId", (q) =>
                q.eq("resendId", event.data.email_id),
            )
            .unique();
...
async function enqueueCallbackIfExists(
    ...
    if (
        event.type === "email.received" &&
        lastOptions?.options.onEmailReceivedEvent
    ) {
        const handle = lastOptions.options.onEmailReceivedEvent
    ...
    if (lastOptions.options.onEmailEvent) {
        const handle = lastOptions.options.onEmailEvent
            .fnHandle as FunctionHandle<
            "mutation",
            {
                id: Id<"emails">;
    ...
    handler: async (ctx, args) => {
        ...
        for (const toAddress of toAddresses) {
            const results = await ctx.db
                .query("emailsWithRecipients")
                .withIndex("by_recipient", (q) => q.eq("recipient", toAddress))
                .collect();
